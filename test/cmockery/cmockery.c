...
/* Returns 1 if the specified values are equal.  If the values are not equal
 * an error is displayed and 0 is returned. */
static int float_equal_display_error(const float left,
                                      const float right, float e) {
    const int equal = ((left - right) < e);
    if (!equal) {
        print_error("%f != %f\n", left, right);
    }
    return equal;
 ...
 * 1 is returned.  If they're not equal an error is displayed and 0 is
 * returned. */
static int stringn_equal_display_error(
        const char * const left, const char * const right, int len) {
    if (strncmp(left, right, len) == 0) {
        return 1;
    }
    print_error("\"%.*s\" != \"%.*s\"\n", len, left, len, right);
    return 0;
 ...
 * equal 1 is returned.  If they're not equal an error is displayed and 0 is
 * returned */
static int stringn_not_equal_display_error(
        const char * const left, const char * const right, int len) {
    if (strncmp(left, right, len) != 0) {
        return 1;
    }
...
/* Returns 1 if the specified values are equal.  If the values are not equal
 * an error is displayed and 0 is returned. */
static int timeval_equal_display_error(const struct timeval* const left,
                                        const struct timeval* const right) {
	const int equal = left->tv_sec == right->tv_sec && left->tv_usec == right->tv_usec;
	if (!equal) {
		print_error("%ld.%.6ld != %ld.%.6ld\n", left->tv_sec, left->tv_usec, right->tv_sec, right->tv_usec);
	}
	return equal;
}


void _assert_timeval_equal(const struct timeval a, const struct timeval b,
                           const char * const file, const int line) {
	if (!timeval_equal_display_error(&a, &b)) {
		_fail(file, line);
	}
}


void _assert_float_equal(const float a, const float b, const float e, const char * const file,
                       const int line) {
    if (!float_equal_display_error(a, b, e)) {
        _fail(file, line);
    }
}
...
void _assert_stringn_equal(const char * const a, const char * const b, int len,
                          const char * const file, const int line) {
    if (!stringn_equal_display_error(a, b, len)) {
        _fail(file, line);
    }
}


void _assert_stringn_not_equal(const char * const a, const char * const b, int len,
                              const char *file, const int line) {
    if (!stringn_not_equal_display_error(a, b, len)) {
        _fail(file, line);
    }
}
